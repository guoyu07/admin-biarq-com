...
namespace app\libraries\li3_u1\extensions\data\source;
use \OAuth;
...
class Ubuntu1api {
	...
	*/
	...
	/*
		...
		* It request type (file or directory) and new item path.
	...
	public function putEmptyItem($type, $path) {
		...
		try {
			...
			$this->oauth->fetch(
				$this->files_api_url . $this->base_path . $path, $kind_e, OAUTH_HTTP_METHOD_PUT);
	...
	public function publishItem($path, $publish = true) {
		...
		try {
			...
			$this->oauth->fetch($this->files_api_url . $this->base_path . $path_e, $response_e,
	...
	public function deleteItem($path) {
		...
		try {
			...
			$this->oauth->fetch(
				$this->files_api_url . $this->base_path . $path_e, " ", OAUTH_HTTP_METHOD_DELETE);
	...
	public function putFileContent($path, $file_content) {
		...
		try {
			...
			$this->oauth->fetch(
				$this->files_content_url . $this->base_path . $path_e, $file_content,
				OAUTH_HTTP_METHOD_PUT);
