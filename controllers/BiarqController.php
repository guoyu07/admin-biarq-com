...
//format date inputs for output on page
        ...
        $full_start_date = date("d-F-y",
            mktime(0, 0, 0, $start_date_parts[1], $start_date_parts[2], $start_date_parts[0]));
        $ga_start_date = date("Y-m-d",
            mktime(0, 0, 0, $start_date_parts[1], $start_date_parts[2], $start_date_parts[0]));

        $full_end_date = date("d-F-y",
            mktime(0, 0, 0, $end_date_parts[1], $end_date_parts[2], $end_date_parts[0]));
        ...
        $requrlvisits = sprintf(
            "https://www.google.com/analytics/feeds/data?ids=%s&metrics=ga:visits,ga:visitors&start-date=%s&end-date=%s",
            $tableId, $start_date, $end_date);
        ...
        $requrl = sprintf(
            "https://www.google.com/analytics/feeds/data?ids=%s&dimensions=ga:source&metrics=ga:visits&filters=ga:medium==referral&start-date=%s&end-date=%s&sort=-ga:visits&max-results=10",
            $tableId, $start_date, $end_date);
        $referralsxml = $analytics->call_api(Session::read('sessionToken'), $requrl);
        $referrers = $analytics->parse_data($referralsxml);

        $cidades = sprintf("https://www.google.com/analytics/feeds/data?ids=%s&dimensions=ga:city&metrics=ga:visits&sort=-ga:visits&start-date=%s&end-date=%s&max-results=20",
            $tableId, $start_date, $end_date);
        $a = $analytics->call_api(Session::read('sessionToken'), $cidades);
        $cidades = $analytics->parse_data($a);

        if ($visits_graph_type == "date") {
            $requrlvisitsgraph = sprintf("https://www.google.com/analytics/feeds/data?ids=%s&dimensions=ga:date&metrics=ga:visits&start-date=%s&end-date=%s",
                $tableId, $start_date, $end_date);
        } else {
            $requrlvisitsgraph = sprintf("https://www.google.com/analytics/feeds/data?ids=%s&dimensions=ga:month,ga:year&metrics=ga:visits&sort=ga:year&start-date=%s&end-date=%s",
                $tableId, $start_date, $end_date);
        }
        ...
        return compact('visitsgraph', 'referrers', 'visits_graph_type', 'full_end_date',
            'full_start_date', 'cidades');
    }
}
